  ...
  <script>
    ...
    async function load(){
      ...
      try{
        ...
        for(const id of ["regulatory","clinical","deals"])document.getElementById(id+"Count").textContent=state.items.filter(x=>x.category===id).length;
        ...
        tabs.querySelectorAll(".tab").forEach(btn=>btn.onclick=()=>{tabs.querySelectorAll(".tab").forEach(x=>x.classList.remove("active"));btn.classList.add("active");state.category=btn.dataset.category;render()});
        document.getElementById("updated").textContent="更新时间："+new Date(data.updatedAt).toLocaleString("zh-CN");
        document.getElementById("sourceStatus").innerHTML='<div class="source"><span>新闻与行业媒体</span><b class="'+(data.failed.length?"warn":"ok")+'">'+(data.failed.length?"部分可用":"正常")+'</b></div><div class="source"><span>监管与审批信息</span><b class="ok">正常</b></div><div class="source"><span>临床与交易监测</span><b class="ok">正常</b></div>';
        render();
      }catch(e){
